...
async (dave, mek, m, { from, body, isOwner }) => {       
 if (config.AUTO_RECORDING === 'true') {
                await dave.sendPresenceUpdate('recording', from);
            }
         } 
   );
    ...
    for (const text in data) {
        if (body.toLowerCase() === text.toLowerCase()) {
            ...
            if (config.AUTO_VOICE === 'true') {
                //if (isOwner) return;        
                await dave.sendPresenceUpdate('recording', from);
                await dave.sendMessage(from, { audio: { url: data[text] }, mimetype: 'audio/mpeg', ptt: true }, { quoted: mek });
            }
        }
    }                
});
...
async (dave, mek, m, { from, body, isOwner }) => {
    if (config.AUTO_TYPING === 'true') {
        await dave.sendPresenceUpdate('composing', from); // send typing 
    }
});
  ...
  try {
    if (config.ALWAYS_ONLINE === "true") {
      ...
      if (isOwner) {
        ...
        await dave.sendPresenceUpdate("unavailable", from);
      }
    }
  } catch (e) {
    console.log(e);

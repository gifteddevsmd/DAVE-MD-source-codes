...
davlo({
    pattern: "ship",
    alias: ["cup", "love"],
    desc: "Randomly pairs the davlo user with another group member.",
    react: "❤️",
    category: "fun",
    filename: __filename,
}, 
async (dave, mek, m, { from, isGroup, groupMetadata, reply }) => {
    try {
        // Ensure davlo is used in a group
        if (!isGroup) {
        ...
        console.error("Error in ship davlo:", e);
...
davlo({
  ...
  category: "tools",
  filename: __filename
}, async (dave, mek, m, { from, quoted, body, args, q, reply }) => {
  try {
    if (!q) {
    ...
    // Message formatting
    let message = `*DAVE-MD FANCY FONTS*:\n\n${fontResults}\n\n> *BY DAVE-MD*`;
...
async (dave, mek, m, { from, reply }) => {
    try {
        ...
        await dave.sendMessage(from, { text: pickupLine }, { quoted: m });

    } catch (error) {
        console.error("Error in pickupline davlo:", error);
...
async (dave, mek, m, { from, isGroup, text, reply }) => {
    try {
